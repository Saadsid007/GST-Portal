...
async function sheetsFrom(rows: NormalizedInvoiceRow[]) {
  ...
  return { hsnB2b: read("hsn(b2b)"), hsnB2c: read("hsn(b2c)"), docs: read("docs") };
}

describe("GSTR-1 workbook: HSN summary (Table 12)", () => {
  it("fills the tax totals behind the shared formula", async () => {
    ...
    const { hsnB2b } = await sheetsFrom([row({}), row({ invoiceNumber: "IN-708" })]);

    const summary = hsnB2b[2]!;
  ...
  it("merges a 4-digit heading into its 6-digit form", async () => {
    ...
    const { hsnB2b } = await sheetsFrom([
      row({ hsnCode: "4419" }),
      row({ hsnCode: "441900", invoiceNumber: "IN-708" }),
    ]);

    const dataRows = hsnB2b.slice(4).filter((r) => String(r[0] ?? "").trim());
    expect(dataRows).toHaveLength(1);
    expect(dataRows[0]![0]).toBe("441900");
    expect(Number(dataRows[0]![6])).toBeCloseTo(2000, 2);
  });

  it("drops a code that classifies nothing", async () => {
    const { hsnB2b } = await sheetsFrom([
      row({ hsnCode: "000000" }),
      row({ hsnCode: "441900", invoiceNumber: "IN-708" }),
    ]);
    ...
    const codes = hsnB2b
      ...
      .filter(Boolean);
    expect(codes).toEqual(["441900"]);
  });

  it("writes zero rather than a blank where there is no tax", async () => {
    // A blank reads as missing data; a zero states the fact.
    const { hsnB2b } = await sheetsFrom([row({})]);
    const dataRow = hsnB2b[4]!;

    expect(dataRow[8]).toBe("0.00"); // central tax
    expect(dataRow[9]).toBe("0.00"); // state tax
    expect(dataRow[10]).toBe("0.00"); // cess
  });

  it("keeps one readable description instead of a joined catalogue", async () => {
    const { hsnB2b } = await sheetsFrom([
      row({ itemDescription: "Wooden serving tray" }),
      row({ itemDescription: "Wooden coaster set", invoiceNumber: "IN-708" }),
    ]);

    const description = String(hsnB2b[4]![1]);
...
describe("GSTR-1 workbook: documents issued (Table 13)", () => {
  it("reports each invoice series on its own row", async () => {
    // One row spanning "2026-2027/57 to IN-1026" describes no series that exists.
    const { docs } = await sheetsFrom([
    ...
    const dataRows = docs.slice(4).filter((r) => String(r[0] ?? "").trim());
    const ranges = dataRows.map((r) => `${r[1]}→${r[2]} (${r[3]})`);

    expect(ranges).toContain("IN-707→IN-1026 (2)");
    expect(ranges).toContain("2026-2027/57→2026-2027/72 (2)");
  });

  it("folds marketplace order ids into one block", async () => {
    // Each order id has a stem unique to itself, which would otherwise put a
    // thousand orders on a thousand rows.
    const orders = ["00016573357568_1", "00561520991748_1", "01025225404736_1"].map((n) =>
      row({ invoiceNumber: n, invoiceType: "B2CS", buyerGstin: "" })
    );
    const { docs } = await sheetsFrom([...orders, row({ invoiceNumber: "IN-707" })]);

    const dataRows = docs.slice(4).filter((r) => String(r[0] ?? "").trim());
    const orderBlock = dataRows.find((r) => String(r[1]).startsWith("000165"));
  ...
  it("keeps credit notes separate from invoices", async () => {
    const { docs } = await sheetsFrom([
      row({ invoiceNumber: "IN-707" }),
    ...
    const dataRows = docs.slice(4).filter((r) => String(r[0] ?? "").trim());
    const creditNotes = dataRows.filter((r) => r[0] === "Credit Note");
