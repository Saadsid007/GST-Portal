...
import { FileSpreadsheet, Sun, Moon, Zap, Sparkles } from "lucide-react";
...
export default function MarketingLayout({ children }: Props) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background font-sans text-foreground selection:bg-primary/20 selection:text-primary">
      {/* Top Banner */}
      <div className="flex items-center justify-center gap-2 bg-gradient-to-r from-primary via-violet-600 to-indigo-600 px-4 py-2 text-center text-xs font-semibold text-white">
        <Sparkles className="size-3.5" />
        <span>
          GSTPilot v2.5 Released — Full support for Amazon MTR v3, Meesho Returns & TCS
          Reconciliation!
        </span>
        <Link href="/convert" className="ml-1 font-bold underline hover:opacity-90">
          Try Now →
        </Link>
      </div>
      ...
      <header className="sticky top-0 z-50 border-b border-border/60 bg-background/80 backdrop-blur-md transition-all">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
          ...
          <Link href="/" className="group flex items-center gap-2.5">
            <div className="flex size-9 items-center justify-center rounded-xl bg-primary font-bold text-primary-foreground shadow-md shadow-primary/20 transition-all group-hover:scale-105">
              <FileSpreadsheet className="size-5" />
            </div>
            <div>
              <span className="block text-lg leading-none font-bold tracking-tight">GSTPilot</span>
