...
export class RoposoParser extends BasePlatformParser {
  info: PlatformInfo = {
    id: "roposo",
    name: "Roposo Clout Report",
    description: "Supports Roposo Clout seller sales & return reports",
    iconName: "Zap",
    badge: "Roposo Clout",
  ...
  autoMap(headers: string[]): ColumnMapping {
    ...
    headers.forEach((original, idx) => {
      const n = norm[idx]!;
      if (n.includes("order_id") || n.includes("invoice_no")) map.invoiceNumber = original;
      ...
      else if (n.includes("total") || n.includes("order_amount")) map.totalValue = original;
    });
    return map;
  }
  ...
  parse(
    ...
    return rawRows.map((row, index) => {
      const invoiceNumber = String(
        this.getValue(row, mapping.invoiceNumber) || `RPS-INV-${index + 1}`
      ).trim();
      ...
      const buyerGstin = String(this.getValue(row, mapping.buyerGstin) || "")
        .trim()
        .toUpperCase();
      const buyerName = String(this.getValue(row, mapping.buyerName) || "Roposo Customer").trim();
