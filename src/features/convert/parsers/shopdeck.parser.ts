...
export class ShopdeckParser extends BasePlatformParser {
  info: PlatformInfo = {
    id: "shopdeck",
    name: "Shopdeck Seller Report",
    description: "Supports Shopdeck D2C orders and GST exports",
  ...
  autoMap(headers: string[]): ColumnMapping {
    ...
    headers.forEach((original, idx) => {
      const n = norm[idx]!;
      if (n.includes("order_id") || n.includes("invoice_number")) map.invoiceNumber = original;
      else if (n.includes("order_date") || n.includes("invoice_date")) map.invoiceDate = original;
      else if (n.includes("buyer_gstin") || n.includes("gstin")) map.buyerGstin = original;
      else if (n.includes("customer_name") || n.includes("buyer_name")) map.buyerName = original;
      else if (n.includes("place_of_supply") || n.includes("shipping_state"))
      ...
      else if (n.includes("taxable_value") || n.includes("subtotal")) map.taxableValue = original;
  ...
  parse(
    ...
    return rawRows.map((row, index) => {
      const invoiceNumber = String(
        this.getValue(row, mapping.invoiceNumber) || `SPD-INV-${index + 1}`
      ).trim();
      ...
      const buyerGstin = String(this.getValue(row, mapping.buyerGstin) || "")
        .trim()
        .toUpperCase();
      const buyerName = String(this.getValue(row, mapping.buyerName) || "Shopdeck Customer").trim();
