...
export class MyntraParser extends BasePlatformParser {
  info: PlatformInfo = {
    id: "myntra",
    name: "Myntra Seller Report",
    description: "Supports Myntra Partner Portal GST reports",
  ...
  parse(
    ...
    return rawRows.map((row, index) => {
      const invoiceNumber = String(
        this.getValue(row, mapping.invoiceNumber) || `MYN-INV-${index + 1}`
      ).trim();
      ...
      const buyerGstin = String(this.getValue(row, mapping.buyerGstin) || "")
        .trim()
        .toUpperCase();
      const buyerName = String(this.getValue(row, mapping.buyerName) || "Myntra Customer").trim();
